    ...
    if hourly:
        if smoothed:
            ...
            if sentiment:
                ...
                # SENTIMENT = ['random']
                # Da 'random' noch nicht in trainierten Modellen, muss hier noch die andere Sentiment list genommmen werden
                SENTIMENT = []
        ...
        if smoothed:
            ...
            path = 'data/daily/daily_testing_data.csv'
            if sentiment:
                model_path = 'model/daily/noSentiment/Turbulence'
                SENTIMENT = ['stocktwitsPosts', 'stocktwitsLikes', 'stocktwitsImpressions', 'stocktwitsSentiment',
                             'random']
            else:
...
def main():
    ...
    path, model_path, SENTIMENT = directory(hourly, sentiment, smoothed)
    ...
    for seed in range(1):
        ...
        result.to_csv(f"result{agent}{seed}_noTresh.csv")
